...
ETL_SERVICE_URL = "http://192.168.0.40:8000"
...
async def get_pdfs_data(user: User = Depends(current_active_user)):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print(f"ETL Service List Error: {e}")
            return []
...
async def upload_pdf(
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(status_code=e.response.status_code, detail=f"ETL Service Error: {e.response.text}")
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Connection to ETL failed: {str(e)}")
