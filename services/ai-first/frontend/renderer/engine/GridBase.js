...
export class GridBase {
    ...
    handleRowDoubleClick(rowId, event) {
        ...
        if (action) {
            const url = (action.url || action.action_url || '').replace('{id}', rowId);

            if (action.action === 'navigate') {
                window.navigateTo(url);
            } else if (action.action === 'modal-form' || action.action === 'edit') {

                let schemaToPass = action.schema;
                if (!schemaToPass && this.container.dataset.schema) {
                    // Fallback to global schema if action specific schema is missing
                    // We need to re-encode it because handleEditAction expects Base64
                    try {
                        schemaToPass = btoa(this.container.dataset.schema);
                    } catch (e) { }
                } else if (typeof schemaToPass !== 'string') {
                    try {
                        schemaToPass = btoa(JSON.stringify(schemaToPass));
                    } catch (e) { }
                }
